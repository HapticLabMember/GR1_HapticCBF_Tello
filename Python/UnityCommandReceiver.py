...
class UnityCommandBridge(Node):
    def __init__(self):
        ...
        self.sub = self.create_subscription(
            Float32MultiArray,
            'unity_cmd',   # matches Unity unifiedTopic (no leading slash)
            self.callback,
            10
        )
    ...
    def callback(self, msg: Float32MultiArray):
        ...
        forward = data[0]   # treat as RC units -100..100
        yaw     = data[1]
        takeoff_flag = data[2]
        land_flag    = data[3]

        # Build Twist for /control
        twist = Twist()
        ...
        # Edge-triggered takeoff / land (Unity sends 1 only on key-down)
        if takeoff_flag > 0.5:
